...
{
    $group_name = $_POST['username'];
    $user_id_to_check = $_REQUEST['id'];
    $sql1 = "UPDATE group_users,account SET group_users.Group_Name =?, account.Group_Name=?  WHERE account.Group_ID=? and group_users.Group_ID = ?";
    mysqli_error($conn);
    if($stmt1 = mysqli_prepare($conn, $sql1))
        mysqli_stmt_bind_param($stmt1, "ssss",$group_name,$group_name, $user_id_to_check, $user_id_to_check);
    ...
    if(mysqli_stmt_execute($stmt1))
    {
        ...
        echo'
        <script type="text/javascript">location.href = "group.php";</script>';
    }
    else {
...
<div class="w3-main" style="margin-left:300px;margin-top:43px;">
    ...
    <div class="topnavx">
        ...
        <a href="#">/</a>
        <a class="active" href="#">Rename Group</a>
    </div>
...
$user_id_to_check = $_REQUEST['id'];
$sqlquery = "SELECT * FROM group_users where Group_ID = ?";
mysqli_error($conn);
...
echo'
    ...
    <div class="col-25">
            <label for="group_ID">Group ID:</label></div>

        <input type="text" id="group_ID" name="group_ID" placeholder="group_ID" value="' .$row['Group_ID'] .'"  required disabled><br>
        
        <div class="col-25">
            <label for="username">Group name:</label></div>

        <input type="text" id="username" name="username" placeholder="Group Name" value="' .$row['Group_Name'] .'" autofocus required>
        ...
        <input type="submit" id="submit" value="Update Group" style="border-radius:20px;
background-color:#1d3056;
margin-left:120px;
margin-top:40px;
margin-bottom:40px;
