...
if(isset($_POST['save'])){
    ...
    $group_ID = $_POST['username'];
    for($i=0;$i<count($checkbox);$i++) {
        $del_id = $checkbox[$i];
        mysqli_query($conn, "UPDATE account SET Group_ID='" . $group_ID . "' WHERE USER_ID='" . $del_id . "'");
        mysqli_query($conn,"UPDATE account INNER JOIN group_users ON account.Group_ID = group_users.Group_ID SET account.Group_Name = group_users.Group_Name");
    }
}
	...
	<div class="w3-overlay w3-hide-large w3-animate-opacity" onclick="w3_close()" style="cursor:pointer" title="close side menu" id="myOverlay"></div>
		<div class="w3-main" style="margin-left:300px;margin-top:43px;">
			...
			<ul class="breadcrumb">
				...
				<li style="font-size:25px;"><a href="#">Groups</a></li>
			</ul>


	<div class="dropdowna">
		<a href="add_group.php" class="btn a" role="button">Add Group</a>
		<!-- <a href="addtogroup.php"  style="float: right" class="btn a" role="button">Add to Group</a>-->
		</div>
    ...
    <hr>
        <div class="panel panel-primary filterable" style="border-color: #1d3056;">
            ...
            <table class="table">
                <thead>
                <tr class="filters">
                    <th><input type="checkbox" id="checkAlgroup"></th>
                    ...
                    <th><input type="text" class="form-control" placeholder="No. of Users" disabled></th>
                    <th><input type="text" class="form-control" placeholder="Options" disabled></th>
                </tr>
                </thead>
                <tbody>';

                $sql = "SELECT * FROM group_users WHERE Group_ID";
                ...
                else
                {
                    ...
                    $query = "SELECT * FROM group_users ORDER BY Group_ID asc";
                }
                ...
                if(mysqli_num_rows($result) > 0)
                {
                    ...
                    while($row = mysqli_fetch_array($result))
                    {
                        ...
                        $sql1 = "SELECT * FROM account WHERE Group_Name='" . $row['Group_Name'] . "'";
                        $result1 = mysqli_query($conn,$sql1);
                        $no_of_users = mysqli_num_rows($result1);


                        $output .= '
                           <tr>
                            <td><input type="checkbox" id="checkItem" name="checkgroup[]" value="'.$row["Group_ID"].'"></td>
                            <td>'.$row["Group_ID"].'</td>
                            ...
                            <div class="dropdown">
						        ...
						        <div class="dropdown-content" style="text-align: left;">
						            <a href="rename_group.php?id=' .$row["Group_ID"]. '">Edit</a>
						            <a href="delete_group.php?id=' .$row["Group_ID"]. '">Delete</a>
						        </div></div>
						        </td>
                           </tr>
                          ';
...
<label for="username" style="margin: 2%">Group ID: </label>
<input type="text" id="username" name="username" placeholder="Group_ID" required>
<span id="Result"> </span>
<p style="float: right; margin: 2%"><button type="submit" class="btn a" name="save">Add To Group</button></p>
            </table>
            </form>
        </div>
    </div>
...
<script>
    $("#checkAlgroup").click(function () {
        $("input[name='checkgroup[]']").not(this).prop('checked', this.checked);
    });
</script>